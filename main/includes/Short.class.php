...
class Short extends App{	
	...
	private function splash($url){
		...
		if(!empty($this->config["timer"]) || $this->config["timer"] !=="0"){
			//Main::add('<script type="text/javascript">var count = '.$this->config['timer'].';var countdown = setInterval(function(){$("a.redirect").attr("href","#pleasewait").html(count + " seconds");if (count < 1) {clearInterval(countdown);$("a.redirect").attr("href","'.$url->url.'").html("Redirect me");}count--;}, 1000);</script>',"custom",FALSE);     
